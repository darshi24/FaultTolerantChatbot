...
public class BotServer extends UnicastRemoteObject implements BotServerInterface {
    ...
    public BotServer(List<String> portsList, int index, String crashTag, String crashPort1, String crashPort2) throws RemoteException {
        ...
        try{
            ...
            System.out.println("Another service already present on this port. Please start " +
                    "on a different port.");
        }
    }
    ...
    public static void main(String[] args) throws RemoteException {
        ...
        for(int i = 0; i < 5; i++) {
            if(!CommonUtils.isPortValid(args[i])) {
                System.out.println("Port number " + i + 1 + "is invalid. Run the program again" +
        ...
        BotServer server = new BotServer(portsList,Integer.parseInt(index), crashTag, crashPort1, crashPort2);
    ...
    @Override
    public String createHistory(Long timestamp, String clientTicker, String price) throws RemoteException {
        return prepareAndPropose(timestamp, clientTicker, price);
    }

    @Override
    public String prepareAndPropose(Long timestamp, String clientTicker, String price) throws RemoteException {
        ...
        if(promiseResponses >= MAJORITY){  // send proposal to all acceptors
            ...
            if(promiseResponseParams.length > 2){ // if it received any piggybacked value
                ...
                updatedMessage = promiseResponseParams[2] + " " + promiseResponseParams[3] + " " +
    ...
    @Override
    public boolean proposal(Long proposalId, Long lastAcceptedProposalId, String message) throws RemoteException {
    ...
    private static void checkCrashTagValidity(String arg) {
        if(!(arg.equalsIgnoreCase("crashP1") ||
                arg.equalsIgnoreCase("crashP2"))) {
